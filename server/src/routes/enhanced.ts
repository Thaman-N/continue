// Enhanced routes for Continue.dev-style context providers
...
import { FileApplicator } from '../services/FileApplicator';

export interface EnhancedServices {
  fileService: FileService;
  gitService: GitService;
  enhancedContextService: EnhancedContextService;
  projectService: ProjectService;
}

export function setupEnhancedRoutes(app: Express, services: EnhancedServices) {
  ...
  app.get('/api/context/enhanced', async (req, res) => {
    try {
      const options = {
        ...
        provider: req.query.provider as string || 'codebase',
        query: req.query.query as string || '',
        includeRepoMap: req.query.includeRepoMap === 'true',
        promptFormatting: req.query.format ? {
          provider: req.query.format as string,
          maxTokens: parseInt(req.query.maxTokens as string) || 8000
        } : undefined,
  ...
  app.post('/api/prompt/format', async (req, res) => {
    try {
      ...
      const formattedPrompt = await enhancedContextService.formatPrompt(
        message,
        contextItems,
        options,
        systemPrompt
  ...
  app.get('/api/repo/map', async (req, res) => {
    try {
      const query = req.query.folder as string || 'entire-codebase';
  ...
  app.post('/api/response/process', async (req, res) => {
    try {
      ...
      const responseExtractor = new ResponseExtractor(projectService.getProjectRoot());
      const artifactExtractor = new ArtifactExtractor(projectService.getProjectRoot());
      ...
      console.log('Debug - extractedCode blocks:', extractedCode.map((code: any, i: number) => 
        `Block ${i}: filename=${code.filename || 'NONE'}, language=${code.language}, contentLength=${code.content?.length || 0}`
      ));
      ...
      const fileApplicator = new FileApplicator(projectService.getProjectRoot());
