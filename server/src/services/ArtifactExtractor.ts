// Service for extracting code from provider-specific artifacts and containers
import { CodeBlock, FileChange, ExtractedResponse } from './ResponseExtractor';

export interface ArtifactInfo {
  id: string;
  title: string;
  type: 'code' | 'text' | 'markdown' | 'html';
  language?: string;
  content: string;
  filename?: string;
}
...
export class ArtifactExtractor {
   ...
   */
  extractClaudeArtifacts(htmlContent: string): ArtifactInfo[] {
    ...
    while ((match = artifactRegex.exec(htmlContent)) !== null) {
      ...
      if (codeMatch || textMatch) {
        ...
        artifacts.push({
          ...
          type: (typeMatch ? typeMatch[1] : 'code') as any,
   ...
   */
  extractChatGPTCode(htmlContent: string): ArtifactInfo[] {
    const artifacts: ArtifactInfo[] = [];
   ...
   */
  extractGeminiCode(htmlContent: string): ArtifactInfo[] {
    const artifacts: ArtifactInfo[] = [];
   ...
   */
  extractGenericCode(htmlContent: string): ArtifactInfo[] {
    const artifacts: ArtifactInfo[] = [];
   ...
   */
  convertArtifactsToCodeBlocks(artifacts: ArtifactInfo[]): CodeBlock[] {
   ...
   */
  extractAllArtifacts(htmlContent: string, provider?: string): ArtifactInfo[] {
   ...
   */
  private inferFilenameFromContent(content: string, language?: string): string | undefined {
   ...
   */
  private isLikelyFullFile(content: string): boolean {
