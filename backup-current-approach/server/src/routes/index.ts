...
export function setupRoutes(app: Express, services: Services) {
  ...
  app.get('/api/context', async (req, res) => {
    try {
      const options = {
        maxFiles: parseInt(req.query.maxFiles as string) || 20,
        includeGitStatus: req.query.includeGitStatus !== 'false',
        fileExtensions: req.query.extensions ? (req.query.extensions as string).split(',') : [],
        excludePatterns: req.query.exclude ? (req.query.exclude as string).split(',') : []
      };
  ...
  async function applyFileChange(change: any, fileService: any) {
    const { path, content, type, confidence, reasoning } = change;
    ...
    try {
      if (type === 'create') {
        ...
        if (!exists) {
          ...
          // Check if this is a targeted function change
          if (reasoning && reasoning.includes('Single function implementation detected')) {
            const result = await applyFunctionChange(path, content, fileService);
            return {
              path,
            ...
            return {
              path,
              success: true,
              message: `Modified file ${path} (full replacement)`,
              type: 'modify',
              confidence
            };
          }
  ...
  // Function to apply targeted function changes (Continue-style precise editing)
  async function applyFunctionChange(filePath: string, newFunctionCode: string, fileService: any) {
    try {
      const existingContent = await fileService.readFile(filePath);
      
      // Extract function name from new code
      const functionMatch = newFunctionCode.match(/function\s+(\w+)/);
      if (!functionMatch) {
        // Fallback to full replacement if can't parse function
        await fileService.writeFile(filePath, newFunctionCode);
        return {
          success: true,
          message: `Updated ${filePath} (couldn't parse function, used full replacement)`,
          details: 'Function name not detected'
        };
      }
      
      const functionName = functionMatch[1];
      console.log(`Attempting to replace function: ${functionName}`);
      ...
      if (matches && matches.length === 1) {
        // Replace the specific function
        const updatedContent = existingContent.replace(functionRegex, newFunctionCode.trim());
        await fileService.writeFile(filePath, updatedContent);
        return {
          success: true,
          message: `Updated function ${functionName} in ${filePath}`,
          details: `Replaced ${matches[0].substring(0, 50)}... with new implementation`
        };
      } else if (matches && matches.length > 1) {
        // Multiple matches - append with comment
        const updatedContent = existingContent + '\n\n// Updated implementation:\n' + newFunctionCode;
        await fileService.writeFile(filePath, updatedContent);
        return {
          success: true,
          message: `Added updated function ${functionName} to ${filePath} (multiple matches found)`,
          details: 'Appended due to ambiguous matches'
        };
      } else {
        // Function not found - append to file
        const updatedContent = existingContent + '\n\n' + newFunctionCode;
        await fileService.writeFile(filePath, updatedContent);
        return {
          success: true,
          message: `Added new function ${functionName} to ${filePath}`,
          details: 'Function not found in existing file'
        };
      }
    } catch (error) {
      return {
        success: false,
        message: `Failed to apply function change: ${error instanceof Error ? error.message : 'Unknown error'}`,
        details: 'Error during function replacement'
      };
    }
  }
  ...
  app.post('/api/files/apply', async (req, res) => {
    try {
      ...
      const backupDirectory = path.join(fileService.getProjectRoot(), '.ai-assistant-backups');
  ...
  app.post('/api/search/files', async (req, res) => {
    try {
      ...
      const files = ['package.json', 'src/index.ts', 'README.md'].filter(f => 
        f.includes(pattern) || pattern === '*'
      );
  ...
  function analyzeResponseForFileChanges(fullResponse: string, extractedCode: any[]) {
  ...
  function analyzeCodeBlock(codeBlock: any, fullResponse: string) {
    ...
    // Enhanced detection for partial changes
    ...
    if (contextBefore.includes('line') || contextBefore.includes('section')) {
      score += 15;
      reasoning.push('Context suggests partial modification');
    }

    // Specific patterns for function modifications
    if (contextBefore.includes('implement') && contextBefore.includes('function')) {
      score += 35;
      reasoning.push('Context requests function implementation');
    }

    if (contextBefore.includes('todo') || contextBefore.includes('fixme') || contextBefore.includes('incomplete')) {
      score += 30;
      reasoning.push('Context suggests completing incomplete code');
    }

    if (contextBefore.includes('subtract') || contextBefore.includes('multiply') || contextBefore.includes('divide')) {
      score += 20;
      reasoning.push('Context mentions specific math functions');
    }

    // Detect targeted function replacement vs full file
    if (content.includes('function ') && !content.includes('module.exports') && !content.includes('const ') && content.split('\n').length < 10) {
      score += 25;
  ...
  function getContextBefore(codeBlock: any, fullResponse: string): string {
  ...
  function getContextAfter(codeBlock: any, fullResponse: string): string {
    const blockIndex = fullResponse.indexOf(codeBlock.content);
    if (blockIndex === -1) return '';
    
    const after = fullResponse.substring(blockIndex + codeBlock.content.length, 
