...
const ChangeReviewer: React.FC<ChangeReviewerProps> = ({ isConnected }) => {
  ...
  console.log('ChangeReviewer render - isConnected:', isConnected, 'extractedChanges:', extractedChanges);

  if (!isConnected) {
    console.log('ChangeReviewer: Not connected');
    return (
      <div style={{ 
        padding: '16px', 
        textAlign: 'center', 
  ...
  if (!extractedChanges || extractedChanges.changes.length === 0) {
    console.log('ChangeReviewer: No changes found');
    return (
      <div style={{ 
        padding: '16px', 
        textAlign: 'center', 
  ...
  console.log('ChangeReviewer: Found changes:', extractedChanges.changes);
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '14px', fontWeight: '500' }}>
                {change.path ? change.path.split('/').pop() : 'Unknown file'}
              </div>
              <div style={{ fontSize: '12px', color: '#6b7280' }}>
                {change.path || 'Unknown path'} • {change.type || 'unknown'}
              </div>
            </div>
          </div>
        ))}
